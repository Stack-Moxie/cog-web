...
// Force gRPC clients to reconnect every ~5 minutes so Docker Swarm IPVS round-robin
...
// The GOAWAY is still sent at ~5 minutes (preventing new streams on the old connection),
// but existing streams complete naturally.
//
// Startup jitter (0–60s) is added to max_connection_age_ms so that each container
// instance picks a slightly different connection age. This prevents all workers from
// receiving GOAWAY simultaneously (thundering herd) after a deployment where all
// workers connect to the web cog at the same time.
const connectionAgeJitterMs = Math.floor(Math.random() * 60000);
const server = new TypedServerOverride({
  'grpc.max_connection_age_ms': 300000 + connectionAgeJitterMs,
